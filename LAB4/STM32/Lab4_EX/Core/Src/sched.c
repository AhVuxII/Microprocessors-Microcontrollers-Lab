...
void SCH_Dispatcher(){
	while (Tlist.size != 0 && Tlist.head->Delay == 0){
		...
		// save ptr of current task
		Task_Control* pTaskToReAdd = Tlist.head;

		// remove 1st list task
		Tlist.head = Tlist.head->nextTask;
		Tlist.size--;

		if (pTaskToReAdd->Period == 0) {
			// task only run once -> remove from the list
			free(pTaskToReAdd);
		} else {
			pTaskToReAdd->Delay = pTaskToReAdd->Period;
			pTaskToReAdd->nextTask = NULL;

			addTask(pTaskToReAdd);
		}
	}
